...
1. **系统 2 思考层 (纯 VLM 大脑 / Qwen3-VL-8B):** 引入未被动作数据“污染”的纯视觉语言大模型（本地部署的 Qwen3-VL-8B）作为系统大脑。它保留了极其强大的常识与空间逻辑推理能力。利用我们首创的 **Topo-Graph RAG**（拓扑感知的图检索增强）分析当前 LIBERO 仿真环境视角，提取 `supports`, `occluded_by` 等空间拓扑关系，规划出宏观指令、安全防干涉偏航角 (VLM-Yaw Reasoning) 以及物理对象的 3D 几何包围盒。
