...
(1)~a Hierarchical Affordance-Aware RAG (\haarag{}) that encodes four-dimensional affordance descriptors---affordance type, material, fragility, and graspable region---and retrieves grasp strategies by functional affordance compatibility rather than visual appearance;
(2)~a Scene Graph Constraint Reasoner that constructs spatial relationship graphs from VLM perception and translates spatial constraints into concrete grasp parameter adjustments;
and (3)~an Agentic Self-Reflective Pipeline employing a 14-type failure taxonomy with three-level adaptive retry for closed-loop grasp refinement.
Evaluated on a 12-task benchmark spanning single-grasp, interactive, and long-horizon scenarios with 360 trials per configuration, \sysname{} achieves a 78.3\% overall success rate---a 53.3\,pp absolute improvement over VLM-only baselines---demonstrating that affordance-aware retrieval, scene graph reasoning, and agentic recovery are jointly essential for robust manipulation.
\end{abstract}
