...
class RetrieveExperienceTool(Tool):
    ...
    def execute(self, query: str, top_k: int = 3, **kwargs) -> str:
        if self._retriever is None: return "Error: Retriever not initialized"
...
class AnalyzeObjectTool(Tool):
    ...
    def execute(self, object_name:
        str, focus_aspects: List[str] = None,
                image=None, **kwargs) -> str: if self._vlm_engine is None: # Note
