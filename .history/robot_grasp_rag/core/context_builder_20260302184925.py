...
JSON file pathoutputgrasp_pose: ```json
{
    "grasp_pose": {
        ...
        "orientation": {"qx": float, "qy": float, "qz": float, "qw": float},  // poseEN
...
4. ENgrasp_poseEN"""
    ...
    def _summarize_pose(self, pose: Dict[str, Any]) -> str:
        ...
        return f"positionEN ({pos.get('x', 0):.2f}, {pos.get('y', 0):.2f}, {pos.get('z', 0):.2f})m"
    ...
    def format_as_prompt(
        ...
        if context["examples"]:
            parts.append("## ENsuccessexperience")
            parts.append("")
        ...
        parts.append("## ENtask")
        parts.append("")
        parts.append(f"- taskEN: {context['query']['task']}")
        
        if context['query'].get('target_object'):
            parts.append(f"- target_object: {context['query']['target_object']}")
            
        parts.append(f"- sceneimage: {context['query']['image_ref']}")
        parts.append("")
        parts.append("sample, outputENsceneENtarget_objectENgrasp_pose。")
