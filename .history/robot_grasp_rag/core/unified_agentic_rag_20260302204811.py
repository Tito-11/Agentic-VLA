...
class UnifiedAgenticRAG:
    ...
    def scene_graph_rag(self):
        if self._scene_graph_rag is None:
            try:
                from .scene_graph_rag import SceneGraphRAG
    ...
    def agentic_pipeline(self):
        if self._agentic_pipeline is None:
            try:
                from .agentic_grasp_pipeline import AgenticGraspPipeline
    ...
    def _build_fused_prompt(self, fused: Dict[str, Any]) -> str:
        ...
        for i, exp in enumerate(fused["experiences"][:5]):
            ...
            if isinstance(exp["data"], dict):
                ...
                lines.append(f"- Grasp type: {exp['data'].get('grasp_type', 'unknown')}")
    ...
    def _reflect_on_failure(
        ...
        reflections = [
            "Insufficient grasp force, try firmer grip",
            "Approach path blocked, try side approach",
            "Target position error, refine localization",
            "Object slipped, increase grip friction",
        ]
        
        if attempt < len(reflections):
            return reflections[attempt]
        return "Apply standard grasp strategy"
